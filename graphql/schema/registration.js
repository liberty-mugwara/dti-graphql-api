...
const userExists = async ({ nationalId, throwError = false } = {}) => {
  const exists = await User.model.exists({
    nationalId,
    isDeleted: false,
  });
...
const registrationResolvers = {
  Mutation: {
    async startRegistration(_, { nationalId }, { dataSources }) {
      ...
      const isRegistered = await userExists({ nationalId });
    ...
    async verifyRegistration(
      ...
      const [__, { firstName, lastName, RVC } = {}] = await Promise.all([
        userExists({ nationalId, throwError: true }),
    ...
    async registerUser(
      ...
      await userExists({ nationalId, throwError: true });
